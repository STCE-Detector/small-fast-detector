...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "DetectLateDecoup", "DetectLateDecoupV2"
...
class DetectLateDecoup(nn.Module):
    """YOLOv8 Detect head for detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.coupled_module = nn.ModuleList(
            nn.Sequential(PConv2d(x, 3, forward='split_cat'), Conv(x, c2, 1)) for x in ch
        )
        self.cv2 = nn.ModuleList(
            nn.Conv2d(c2, 4 * self.reg_max, 1) for x in ch
        )
        self.cv3 = nn.ModuleList(
            nn.Conv2d(c2, self.nc, 1) for x in ch
        )
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
    ...
    def forward(self, x):
        """Concatenates and returns predicted bounding boxes and class probabilities."""
        for i in range(self.nl):
            z = self.coupled_module[i](x[i])
            x[i] = torch.cat((self.cv2[i](z), self.cv3[i](z)), 1)
        if self.training:  # Training path
            return x
...
class DetectLateDecoupV2(nn.Module):
    """YOLOv8 Detect head for detection models."""
...
class DetectEmb(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.stride = torch.zeros(self.nl)  # strides computed during build
        self.ch_emb = 64
        self.no = nc + self.reg_max * 4 + self.ch_emb  # number of outputs per anchor
        if len(ch) == 4:
            # c2, c3 = max((16, ch[1] // 4, self.reg_max * 4)), max(ch[1], min(self.nc, 100))  # New, heavier version
            c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))     # Original, ligther version
        else:
        ...
        # TODO: maybe cv4 should be simpler and faster, add bias
    ...
    def forward(self, x):
        ...
        if self.export and self.format in ("saved_model", "pb", "tflite", "edgetpu", "tfjs"):  # avoid TF FlexSplitV ops
            ...
            box, cls, emb = x_cat.split((self.reg_max * 4, self.nc, self.ch_emb), 1)
        dbox = self.decode_bboxes(box)
        ...
        if self.export and self.format in ("tflite", "edgetpu"):
            ...
            norm = self.strides / (self.stride[0] * img_size)
            dbox = dist2bbox(self.dfl(box) * norm, self.anchors.unsqueeze(0) * norm[:, :2], xywh=True, dim=1)
        y = torch.cat((dbox, cls.sigmoid(), emb), 1)
        return y if self.export else (y, x)
